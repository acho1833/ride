...
const DashboardIsolatedComponent = ({ data }: Props) => {
  ...
  return (
    <DashboardSectionComponent title="Isolated Entities" tooltip={SECTION_TOOLTIPS.isolatedEntities} badge={badgeText}>
      {data.length === 0 ? (
        <p className="text-muted-foreground text-xs">No isolated entities</p>
      ) : (
        <div className="flex flex-wrap gap-2">
          {data.map(entity => (
            <div key={entity.id} className="border-border flex items-center gap-1.5 rounded-md border px-2 py-1">
