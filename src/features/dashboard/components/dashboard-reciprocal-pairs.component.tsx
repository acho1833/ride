...
const DashboardReciprocalPairsComponent = ({ data }: Props) => {
  return (
    <DashboardSectionComponent title="Reciprocal Pairs" tooltip={SECTION_TOOLTIPS.reciprocalPairs} badge={data.length}>
      {data.length === 0 ? (
        <p className="text-muted-foreground text-xs">No data</p>
      ) : (
        <div className="space-y-2">
          {data.map(pair => (
            <div key={`${pair.entityA.id}-${pair.entityB.id}`} className="border-border space-y-1 border-b pb-2 last:border-b-0">
