...
const DashboardDegreeDistributionComponent = ({ data, median, max }: Props) => {
  ...
  return (
    <DashboardSectionComponent title="Degree Distribution" tooltip={SECTION_TOOLTIPS.degreeDistribution}>
      {data.length === 0 ? (
        ...
        <>
          <div className="space-y-1.5">
            {data.map(bucket => (
              <DashboardBarComponent key={bucket.range} label={bucket.range} value={bucket.count} maxValue={maxCount} />
            ))}
          </div>
