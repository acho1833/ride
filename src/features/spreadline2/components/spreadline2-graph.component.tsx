 ...
 *
 * No internal/external distinction — all non-ego nodes use single orange color.
 *
 * Architecture:
...
import {
  ...
  SPREADLINE2_LINE_COLOR,
  GRAPH2_HOP1_LINK_DISTANCE,
  GRAPH2_HOP2_LINK_DISTANCE,
  GRAPH2_HOP1_RADIAL_RADIUS,
  GRAPH2_HOP2_RADIAL_RADIUS,
  GRAPH2_RADIAL_STRENGTH,
  GRAPH2_TIME_TRANSITION_MS
} from '@/features/spreadline2/const';
import { transformSpreadlineToGraph, transformSpreadlineToGraphByTimes } from '@/features/spreadline2/utils';
import type { Spreadline2GraphNode, Spreadline2GraphLink } from '@/features/spreadline2/utils';
...
/** Get fill color for a node — all non-ego use single orange color */
const getNodeFill = (d: Spreadline2GraphNode): string => {
  if (d.isEgo) return EGO_NODE_COLOR;
  return SPREADLINE2_LINE_COLOR;
};

/** Get the radius for a node (ego is larger) */
const getNodeRadius = (d: Spreadline2GraphNode): number => (d.isEgo ? GRAPH_CONFIG.nodeRadius * EGO_SCALE : GRAPH_CONFIG.nodeRadius);

/** Get the icon size for a node (ego is larger) */
const getNodeIconSize = (d: Spreadline2GraphNode): number => (d.isEgo ? GRAPH_CONFIG.iconSize * EGO_SCALE : GRAPH_CONFIG.iconSize);

/** Append visual elements (rect, icon, label) to a node <g> selection */
const appendNodeVisuals = (selection: d3.Selection<SVGGElement, Spreadline2GraphNode, SVGGElement, unknown>) => {
  // Rounded rectangle
  selection
    .append('rect')
    ...
    .attr('filter', d => (d.isEgo ? 'url(#sl2-ego-glow)' : null));
...
const getHopLinkDistance = (link: Spreadline2GraphLink): number => {
  const source = link.source as Spreadline2GraphNode;
  const target = link.target as Spreadline2GraphNode;
  const srcHop = source.hopDistance ?? 0;
  const tgtHop = target.hopDistance ?? 0;
  const maxHop = Math.max(srcHop, tgtHop);
  return maxHop >= 2 ? GRAPH2_HOP2_LINK_DISTANCE : GRAPH2_HOP1_LINK_DISTANCE;
};

/** Get radial radius for a node's hop distance */
const getRadialRadius = (d: Spreadline2GraphNode): number => {
  if (d.hopDistance === 0) return 0;
  if (d.hopDistance === 1) return GRAPH2_HOP1_RADIAL_RADIUS;
  return GRAPH2_HOP2_RADIAL_RADIUS;
};

/** BFS: compute distances from a start node to all reachable nodes */
const bfsDistances = (startId: string, links: Spreadline2GraphLink[]): Map<string, number> => {
...
const Spreadline2GraphComponent = ({ selectedTimes = [], pinnedEntityNames = [] }: Props) => {
  const svgRef = useRef<SVGSVGElement>(null);
  const containerRef = useRef<HTMLDivElement>(null);
  const zoomRef = useRef<d3.ZoomBehavior<SVGSVGElement, unknown> | null>(null);
  const nodesRef = useRef<Spreadline2GraphNode[]>([]);
  const transformRef = useRef<d3.ZoomTransform>(d3.zoomIdentity);
  const [dimensions, setDimensions] = useState<{ width: number; height: number } | null>(null);

  // D3 selection refs for time-change effect
  const gRef = useRef<d3.Selection<SVGGElement, unknown, null, undefined> | null>(null);
  const simulationRef = useRef<d3.Simulation<Spreadline2GraphNode, Spreadline2GraphLink> | null>(null);
  ...
  // Fetch data (same query as Spreadline2Component — React Query deduplicates)
  const {
    data: rawData,
    isPending,
    isError,
  ...
  useEffect(() => {
    ...
    g.append('rect')
      ...
      .attr('width', extent)
      .attr('height', extent)
      .attr('fill', 'url(#sl2-dot-grid-pattern)')
      .attr('pointer-events', 'none');
  ...
  useEffect(() => {
    ...
    const nodes: Spreadline2GraphNode[] = graphData.nodes.map(n => {
    ...
    const links: Spreadline2GraphLink[] = graphData.links.map(l => ({ ...l }));
    ...
    const linkJoin = g
      ...
      .selectAll<SVGLineElement, Spreadline2GraphLink>('line')
      .data(links, (d: Spreadline2GraphLink) => {
    ...
    linkJoin
      ...
      .transition()
      .duration(GRAPH2_TIME_TRANSITION_MS)
    ...
    linkEnter.transition().duration(GRAPH2_TIME_TRANSITION_MS).attr('stroke-opacity', GRAPH_CONFIG.linkStrokeOpacity);
    ...
    const nodeJoin = g
      ...
      .selectAll<SVGGElement, Spreadline2GraphNode>('g')
      .data(nodes, (d: Spreadline2GraphNode) => d.id);
    ...
    nodeEnter.transition().duration(GRAPH2_TIME_TRANSITION_MS).style('opacity', 1);
    ...
    nodeJoin.select('rect').transition().duration(GRAPH2_TIME_TRANSITION_MS).attr('fill', getNodeFill);
    ...
    // ─── Drag behavior ─────────────────────────────────────────────────
    const drag = d3
      .drag<SVGGElement, Spreadline2GraphNode>()
      .on('start', function () {
        this.setAttribute('cursor', 'grabbing');
      })
      .on('drag', function (event: d3.D3DragEvent<SVGGElement, Spreadline2GraphNode, Spreadline2GraphNode>, d) {
        d.x = event.x;
        d.y = event.y;
        ...
        if (connectedLinks) {
          for (const linkEl of connectedLinks) {
            const linkData = d3.select<SVGLineElement, Spreadline2GraphLink>(linkEl).datum();
            const source = linkData.source as Spreadline2GraphNode;
            const target = linkData.target as Spreadline2GraphNode;
    ...
    const simulation = d3
      .forceSimulation<Spreadline2GraphNode>(nodes)
      .force(
        'link',
        d3
          .forceLink<Spreadline2GraphNode, Spreadline2GraphLink>(links)
          .id(d => d.id)
      ...
      .force(
        'collision',
        d3.forceCollide<Spreadline2GraphNode>().radius(d => getNodeRadius(d) + GRAPH_CONFIG.collisionPadding)
      );
    ...
    if (useHopLayout) {
      simulation.force(
        'radial',
        d3.forceRadial<Spreadline2GraphNode>(getRadialRadius, width / 2, height / 2).strength(GRAPH2_RADIAL_STRENGTH)
      );
    }
    ...
    if (isFirstRender) {
      ...
      linkMerged
        .attr('x1', d => (d.source as Spreadline2GraphNode).x ?? 0)
        .attr('y1', d => (d.source as Spreadline2GraphNode).y ?? 0)
        .attr('x2', d => (d.target as Spreadline2GraphNode).x ?? 0)
        .attr('y2', d => (d.target as Spreadline2GraphNode).y ?? 0);
  ...
  useEffect(() => {
    ...
    g.select('.nodes').selectAll<SVGGElement, Spreadline2GraphNode>('g').filter(visibleNodeFilter).style('opacity', null);
    g.select('.nodes')
      .selectAll<SVGGElement, Spreadline2GraphNode>('g')
      ...
      .each(function (d) {
        ...
        node
          ...
          .attr('filter', d.isEgo ? 'url(#sl2-ego-glow)' : null);
        node
          .select('use')
          .attr('x', -iconSize / 2)
          .attr('y', -iconSize / 2)
    ...
    const currentLinks: Spreadline2GraphLink[] = [];
    g.select('.links')
      .selectAll<SVGLineElement, Spreadline2GraphLink>('line')
      .filter(visibleLinkFilter)
      .each(function (d) {
    ...
    g.select('.nodes')
      ...
      .each(function () {
        d3.select(this).select('rect').transition().duration(GRAPH2_TIME_TRANSITION_MS).attr('fill', EGO_NODE_COLOR);
      });
...
export default Spreadline2GraphComponent;
