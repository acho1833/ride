...
async function loadGoogleOrgData() {
  const SQL = await initSqlJs();
  const buffer = readFileSync(resolve(process.cwd(), 'src/lib/mock-data/mock.db'));
  ...
  const entities: Array<{ id: string; labelNormalized: string; type: string }> = [];
  while (entitiesStmt.step()) {
    entities.push(entitiesStmt.getAsObject() as { id: string; labelNormalized: string; type: string });
  }
  entitiesStmt.free();
  ...
  const relationships: Array<{ sourceEntityId: string; relatedEntityId: string }> = [];
  while (relStmt.step()) {
    relationships.push(relStmt.getAsObject() as { sourceEntityId: string; relatedEntityId: string });
  }
  relStmt.free();
...
let googleOrgData: Awaited<ReturnType<typeof loadGoogleOrgData>>;

beforeAll(async () => {
  googleOrgData = await loadGoogleOrgData();
});
